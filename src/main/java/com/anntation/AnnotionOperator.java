...
public class AnnotionOperator {
    public static void main(String[] args) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException, ClassNotFoundException {
        ...
        element.setName1("name");
        element.setSex1("sex1");
        ...
        for(Annotation fd:annotation){
        	System.out.println(fd.annotationType());
        }
        
        
        Field[] field=SayHiEmlement.class.getDeclaredFields();
        for(Field fd:field){
        	System.out.println(fd.getName()+"---------------"+fd.get(element));
        }
